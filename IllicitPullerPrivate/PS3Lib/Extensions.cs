...
namespace PS3Lib
{
    ...
    public class Extension
    {
        ...
        {
            return Convert.ToBoolean(ReadByte(offset));
        }
        ...
        {
            byte[] buff = Encoding.UTF8.GetBytes(input + "\0");
        ...
        private void SetMem(uint Address, byte[] buffer, SelectAPI API)
        {
            switch (API)
            {
                case SelectAPI.ControlConsole: Common.CcApi.SetMemory(Address, buffer); return;
                case SelectAPI.TargetManager: Common.TmApi.SetMemory(Address, buffer); return;
            }
        }
