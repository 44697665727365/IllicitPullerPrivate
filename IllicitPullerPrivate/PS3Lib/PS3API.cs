...
//                                                   //
// Credits : Enstone, Buc-ShoTz, SkullMods           //
//                                                   //
// Follow me :                                       //
//                                                   //
...
//                                                   //
// ************************************************* //
//   SkullMods- changed statements                   //
// ************************************************* //

using static IllicitPullerPrivate.Classes.Connection;
...
namespace PS3Lib
{
    ...
    public class PS3API
    {
        ...
        private void MakeInstanceAPI(SelectAPI API)
        {
            switch (API)
            {
                case SelectAPI.ControlConsole: if (Common.CcApi == null) Common.CcApi = new CCAPI(); return;
                case SelectAPI.TargetManager: if (Common.TmApi == null) Common.TmApi = new TMAPI(); return;
            }
        }
        ...
        {
            switch (SetAPI.API == SelectAPI.TargetManager)
            {
                case true: Common.TmApi.InitComms(); return;
            }
        }
        ...
        public bool ConnectTarget(int target = 0)
        {
            ...
            return SetAPI.API == SelectAPI.TargetManager ? Common.TmApi.ConnectTarget(target) : new ConsoleList(this).Show();
        }
        ...
        public bool ConnectTarget(string ip)
        {
            ...
            MakeInstanceAPI(GetCurrentAPI());
            switch (Common.CcApi.SUCCESS(Common.CcApi.ConnectTarget(ip)))
            {
                case true: targetIp = ip; return true;
        ...
        public void DisconnectTarget()
        {
            switch (SetAPI.API)
            {
                case SelectAPI.ControlConsole: Common.CcApi.DisconnectTarget(); return;
                case SelectAPI.TargetManager: Common.TmApi.DisconnectTarget(); return;
            }
        }
        ...
        public bool AttachProcess()
        {
            // We'll check again if the instance has been done.
            MakeInstanceAPI(GetCurrentAPI());
            return SetAPI.API == SelectAPI.TargetManager ? Common.TmApi.AttachProcess() : Common.CcApi.SUCCESS(Common.CcApi.AttachProcess());
        }
        ...
        public string GetConsoleName()
        {
            switch (SetAPI.API)
            {
                case SelectAPI.ControlConsole:
                    switch (targetName != string.Empty)
                    {
                        case true:
                    ...
                    return targetName;
                case SelectAPI.TargetManager: return Common.TmApi.SCE.GetTargetName();
                default:
                    return "";
            }
        }
        ...
        public void SetMemory(uint offset, byte[] buffer)
        {
            switch (SetAPI.API)
            {
                case SelectAPI.ControlConsole: Common.CcApi.SetMemory(offset, buffer); return;
                case SelectAPI.TargetManager: Common.TmApi.SetMemory(offset, buffer); return;
            }
        }
        ...
        public void GetMemory(uint offset, byte[] buffer)
        {
            switch (SetAPI.API)
            {
                case SelectAPI.ControlConsole: Common.CcApi.GetMemory(offset, buffer); return;
                case SelectAPI.TargetManager: Common.TmApi.GetMemory(offset, buffer); return;
            }
        }
        ...
        public byte[] GetBytes(uint offset, int length)
        {
            ...
            switch (SetAPI.API)
            {
                case SelectAPI.ControlConsole: Common.CcApi.GetMemory(offset, buffer); break;
                case SelectAPI.TargetManager: Common.TmApi.GetMemory(offset, buffer); break;
            }
            return buffer;
        }
        ...
        {
            return SetAPI.API == SelectAPI.TargetManager ? Enum.GetName(typeof(SelectAPI), SelectAPI.TargetManager).Replace("Manager", " Manager") : Enum.GetName(typeof(SelectAPI), SelectAPI.ControlConsole).Replace("Console", " Console");
        }
