...
namespace PS3Lib.NET
{
    ...
    public class PS3TMAPI
    {
        ...
        {
            return Is32Bit() == true ? InitTargetCommsX86() : InitTargetCommsX64();
        }

        public static SNRESULT Connect(int target, string application)
        {
            return Is32Bit() == true ? ConnectX86(target, application) : ConnectX64(target, application);
        }

        public static SNRESULT PowerOn(int target)
        {
            return Is32Bit() == true ? PowerOnX86(target) : PowerOnX64(target);
        }

        public static SNRESULT PowerOff(int target, bool bForce)
        {
            return Is32Bit() == true ? PowerOffX86(target, bForce ? (uint)1 : 0) : PowerOffX64(target, bForce ? (uint)1 : 0);
        }
        ...
        {
            return Is32Bit() == true ? ProcessAttachX86(target, (uint)unit, processID) : ProcessAttachX64(target, (uint)unit, processID);
        }

        public static SNRESULT ProcessContinue(int target, uint processID)
        {
            return Is32Bit() == true ? ProcessContinueX86(target, processID) : ProcessContinueX64(target, processID);
        }
        ...
        {
            return Is32Bit() == true ? GetTargetFromNameX86(new ScopedGlobalHeapPtr(AllocUtf8FromString(name)).Get(), out target) : GetTargetFromNameX64(new ScopedGlobalHeapPtr(AllocUtf8FromString(name)).Get(), out target);
        }
        ...
        {
            return Is32Bit() == true ? ResetX86(target, (ulong)resetParameter) : ResetX64(target, (ulong)resetParameter);
        }
        ...
        {
            return Is32Bit() == true ? ProcessGetMemoryX86(target, unit, processID, threadID, address, buffer.Length, buffer) : ProcessGetMemoryX64(target, unit, processID, threadID, address, buffer.Length, buffer);
        }

        public static SNRESULT ProcessSetMemory(int target, UnitType unit, uint processID, ulong threadID, ulong address, byte[] buffer)
        {
            return Is32Bit() == true ? ProcessSetMemoryX86(target, unit, processID, threadID, address, buffer.Length, buffer) : ProcessSetMemoryX64(target, unit, processID, threadID, address, buffer.Length, buffer);
        }

        public static SNRESULT Disconnect(int target)
        {
            return Is32Bit() == true ? DisconnectX86(target) : DisconnectX64(target);
        }
    }
}
